
import {
  Popover,
  PopoverHandler,
  PopoverContent,
  Typography,
  List,
  ListItem,
  ListItemPrefix,
} from "@material-tailwind/react";
import { FaDiscord, FaLinkedin, FaPhone } from "react-icons/fa";
import { BiLogoGmail } from "react-icons/bi"

export const Contact = () => {
  return (
    <Popover placement="bottom">
      <PopoverHandler>
      <li className="flex items-center gap-2 text-gray-50 font-normal hover:text-orange-300 transition-all duration-500 cursor-pointer"><FaPhone className="text-xl"/> Contatos</li>
      </PopoverHandler>
      <PopoverContent className="w-72 p-3 bg-neutral-950 border-none">
        <div className="mb-4 flex items-center gap-4 border-b border-orange-400 pb-4">
          <div>
            <Typography className="text-orange-400 font-cursive text-3xl">
              Contatos
            </Typography>
            <Typography variant="small" className="text-gray-300">
              Converse comigo!
            </Typography>
          </div>
        </div>
        <List className="p-0">
          <a className="text-gray-200" target="_blank" href="mailto:rianvitorlhe@gmail.com">
            <ListItem>
              <ListItemPrefix>
                <BiLogoGmail className="mr-2 text-xl text-red-600" />
              </ListItemPrefix>
              Gmail
            </ListItem>
          </a>


          <a className="text-gray-200" target="_blank" href="https://www.aedin.com/in/rian-vitor-dos-santos-de-almeida-a036aa202/">
            <ListItem>
              <ListItemPrefix>
                <FaLinkedin className="mr-2 text-xl text-blue-500" />
              </ListItemPrefix>
              Linkedin
            </ListItem>
          </a>


          <a className="text-gray-200" target="_blank" href="https://discord.com/channels/@me">
            <ListItem>
              <ListItemPrefix>
                  <FaDiscord className="mr-2 text-xl text-indigo-600"/>
              </ListItemPrefix>
              Discord (RianVitor26#0458)
            </ListItem>
          </a>
        </List>
      </PopoverContent>
    </Popover>
  );
}